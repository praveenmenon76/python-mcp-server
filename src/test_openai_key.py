...
def test_openai_key(api_key, verbose=False):
    ...
    if verbose:
        logger.debug(f"API key format: starts with '{api_key[:6]}...', length: {len(api_key)}")
    ...
    for endpoint in endpoints:
        try:
            ...
            if response.status_code == 200:
                ...
                return {"status": "success", "message": "API key is valid and working correctly!"}
                ...
                try:
                    ...
                    return {"status": "error", "message": f"API error (HTTP {response.status_code}): {error_message}"}
                except:
                    return {"status": "error", "message": f"API error (HTTP {response.status_code}): {response.text[:200]}"}
...
def main():
    ...
    parser.add_argument('--from-env', action='store_true', help='Use OPENAI_API_KEY environment variable')
    parser.add_argument('--interactive', '-i', action='store_true', help='Enter API key interactively (more secure)')
    ...
    if args.interactive:
        ...
        try:
            ...
            if not config_path.exists():
                ...
                logger.info("Please provide an API key using --api-key, --from-env, or --interactive")
                return 1
    ...
    if result["status"] == "success":
        ...
        logger.info("Your API key is working correctly and can be used with the LLM tool.")
        return 0
    else:
        logger.error(f"❌ Error: {result['message']}")
        logger.info("\nTroubleshooting tips:")
        logger.info("1. Make sure your API key starts with 'sk-' and doesn't have extra quotes or spaces")
        logger.info("2. Check if your OpenAI account has billing set up and sufficient credit")
        logger.info("3. Try generating a new API key from https://platform.openai.com/api-keys")
        logger.info("4. Verify your internet connection and that api.openai.com is accessible")
        return 1
