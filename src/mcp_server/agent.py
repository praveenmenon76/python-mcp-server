...
class IntentAgent:
    ...
    def process_query(self, query: str, context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        if self.use_enhanced_responses and self.llm_tool and self.llm_tool.api_key:
            enhanced_response = self._generate_enhanced_response(query, raw_response, tool_name)
    ...
    def _determine_intent(self, query: str, context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """
        ...
        if self.llm_tool and self.llm_tool.api_key:
            ...
            llm_result = self.llm_tool.process_query(query, context=context, tools_info=tools_info)
            
            if llm_result["status"] == "success":
                return llm_result
            else:
                logger.warning(f"LLM intent recognition failed: {llm_result['message']}")
    ...
    def _generate_enhanced_response(self, query: str, tool_response: Dict[str, Any], tool_name: str) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            llm_result = self.llm_tool.process_enhanced_response(enhanced_prompt, context)
            ...
            logger.warning("LLM enhanced response generation failed or returned unexpected format")
            return None
