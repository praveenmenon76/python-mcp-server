...
class StockPriceTool:
    ...
    def _get_ticker_symbol(self, input_text):
        ...
        if input_text.isupper() and 1 <= len(input_text) <= 5:
            ...
            if input_text.lower() in self.company_to_ticker:
                ...
                logger.info(f"Converted uppercase '{input_text}' to ticker symbol '{ticker}'")
                return ticker
            return input_text
    ...
    def get_stock_price(self, symbol):
        ...
        """
        if not self.api_key:
            return {"status": "error", "message": "API key not configured. Please set ALPHAVANTAGE_API_KEY environment variable."}
            
        # Convert company name to ticker symbol if needed
        ticker = self._get_ticker_symbol(symbol)
        
        if not ticker:
            return {"status": "error", "message": "Please provide a valid company name or stock symbol."}
