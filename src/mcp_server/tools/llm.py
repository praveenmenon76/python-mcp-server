...
import builtins, types as pytypes
...
class LLMTool:
    ...
    def __init__(self):
        ...
        if not self.enabled:
            ...
            logger.info(f"LLM Tool initialized with provider: {self.provider}, model: {self.model}")
            if self.api_key:
                ...
                logger.debug(f"API key format: {masked_key}, length: {len(self.api_key)}")
            ...
            if config_path.exists():
                ...
                for tool in config.get("tools", []):
                    if tool.get("name") == "LLMTool":
                        ...
                        if settings["provider"] == "openai" and "provider" in tool_settings:
                            settings["provider"] = tool_settings.get("provider")
                        if settings["model"] == "gpt-3.5-turbo" and "model" in tool_settings:
                            settings["model"] = tool_settings.get("model")
                        if not settings["azure_endpoint"] and "azure_endpoint" in tool_settings:
                            settings["azure_endpoint"] = tool_settings.get("azure_endpoint")
        ...
        try:
            ...
            if hasattr(e, 'response') and e.response is not None:
                try:
                    ...
                    error_details = f"{error_details} - Response: {json.dumps(response_json)}"
                except:
                    error_details = f"{error_details} - Status code: {e.response.status_code}, Content: {e.response.text[:200]}"
            
            return {"status": "error", "message": f"LLM API request failed: {error_details}"}
        except Exception as e:
    ...
    def _call_openai_api(self, system_prompt, user_message):
        ...
        if response.status_code != 200:
            try:
                ...
                logger.error(f"OpenAI API error ({response.status_code}): {error_type} - {error_message}")
                raise requests.exceptions.HTTPError(f"OpenAI API error: {error_message}")
            except json.JSONDecodeError:
                logger.error(f"OpenAI API returned non-JSON response with status {response.status_code}")
    ...
    def _call_azure_api(self, system_prompt, user_message):
        ...
        if not self.endpoints["azure"]:
            return {"status": "error", "message": "Azure OpenAI endpoint not configured"}
    ...
    def test_api_key(self):
        ...
        try:
            ...
            if hasattr(e, 'response') and e.response is not None:
                try:
                    ...
                    if "error" in response_json:
                        error_details = f"{error_details} - {json.dumps(response_json['error'])}"
                    else:
            ...
            return {
                "status": "error",
                "message": f"API test failed: {error_details}"
            }
        except Exception as e:
            return {
                "status": "error",
    ...
    def process_enhanced_response(self, prompt, context):
        ...
        user_message += f"Response status: {context.get('response_status', 'unknown')}\n"
        
        # Add the raw message from the tool response if available
        if context.get('response_message'):
            user_message += f"Raw response message: {context.get('response_message')}\n\n"
        
        # Add raw data if available
        if context.get('response_data'):
            user_message += f"Response data: {json.dumps(context.get('response_data'))}\n\n"
        elif context.get('tool_response') and isinstance(context.get('tool_response'), dict) and context.get('tool_response').get('data'):
        ...
        try:
            ...
            return {"status": "error", "message": f"Error generating enhanced response: {str(e)}"}