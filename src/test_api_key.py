...
def main():
    ...
    parser.add_argument('--api-key', type=str, help='LLM API key to test (overrides config)')
    parser.add_argument('--provider', type=str, default='openai', choices=['openai', 'azure', 'anthropic'],
    ...
    try:
        ...
        logger.error("Failed to import LLMTool. Make sure you're running this script from the project root.")
        sys.exit(1)
    ...
    logger.info(f"API key: {llm_tool.api_key[:5]}{'*' * 10}{llm_tool.api_key[-4:] if llm_tool.api_key else ''}")
